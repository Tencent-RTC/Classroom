...
const streamStyle: Ref<Record<string, any>> = ref({
  width: '0',
  height: '0',
  '--stream-margin': `${singleStreamMargin}px`,
});
...
function handleHorizontalInfinityLayout() {
  ...
  const streamListContainerHeight = getContentSize(
    streamListContainerRef.value
  ).height;
  const contentHeight =
    (streamListContainerHeight - singleStreamMargin * (props.row - 1)) /
    props.row;
...
function handleVerticalInfinityLayout() {
  ...
  const streamListContainerWidth = getContentSize(
    streamListContainerRef.value
  ).width;
  const contentWidth =
    (streamListContainerWidth - singleStreamMargin * (props.column - 1)) /
    props.column;
...
.equal-points-layout {
  ...
  .stream-list {
    ...
    .stream-list-item {
      margin: calc(var(--stream-margin) / 2);
    }
  }
}
...
.horizontal-infinity-layout {
  ...
  .stream-list-item {
    margin-right: var(--stream-margin);
  }
}
...
.vertical-infinity-layout {
  ...
  .stream-list {
    ...
    .stream-list-item {
      margin-bottom: var(--stream-margin);
    }
  }
}
</style>
